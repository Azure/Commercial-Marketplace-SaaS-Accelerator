...
#.\DeployCliFull.ps1 -WebAppNamePrefix saasamp1 -Location canadacentral
...
#this is needed because how string interpolation works in AZCLi + Powershell -> trying to set this as a string directly in the az cli command will fail because of the ')' character.
...
if(!$checkKV){
    ...
    Write-host "📜  -- KeyVault was present. Skipped creation!"
}
...
$checkSQL = (az sql server list | ConvertFrom-Json).name.Contains($SQLServerName)
if (!$checkSQL){
    ...
    az keyvault secret set `
        ...
        --output none
    Write-host "📜  -- SQL Connection string secret added to keyvault"
    Write-host "📜  --SQL Server created!"
}
else {
     Write-host "📜  --SQL Server was present. Skiped creation!"
}
