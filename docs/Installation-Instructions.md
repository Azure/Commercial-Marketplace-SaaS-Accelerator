...
⚠️Caution: This will deploy/upgrade to the `<branch-to-deploy>` specified while running the upgrade command, which will replace the current version. If you have any custom changes, we recommend you implement a strategy to backup and replace them after the upgrade.
